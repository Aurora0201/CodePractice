...
为了添加远程仓库首先我们需要密钥，下面是添加密钥的代码，注意，添加密钥前确保我们的机器已经注册了

```c++
$ ssh-keygen -t rsa -C "email" //回车三次
```

有了密钥后我们需要先进入储存密钥的地方然后读取密钥，再在github里添加我们的密钥，这样我们就能访问我们的github了，下面是查看密钥的代码
...
在github中添加完密钥后我们就能添加到我们的远程仓库了，注意，需要先创建密钥再连接远程仓库，不然会出现错误，下面是连接代码

```
$ git remote add origin "ssh"
```

这样我们就相当于连接了远程仓库，我们就能进行`push`和`clone`的操作了
...
假如有两台或更多台机器对远程仓库有贡献，那么当我们机器上的master分支落后于远程仓库，我们需要同步更新我们的本地仓库，用下面的命令全部取回本地

```
$ git fetch
```

当远程有新的分支时，我们用下面的命令查看分支，发现远程的分支已经可以看见了

```
$ git branch -a
```

这里我们可以用下面的指令将远程分支拉回本地

```
$ git branch 远程分支名 origin/远程分支
```
